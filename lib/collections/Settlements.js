...
Settlements .deny({
    update: function(userId, settlement, fieldNames){
        ...
        return (_.without(fieldNames, 'dateOrdered', 'dateSettlement', 'category', 'orderSummary', 'orderPrice', 'purchaser', 'communalPurchaser').length > 0);
    }
});
...
validateSettlement = function(settlement){
    ...
    if(!settlement.communalPurchaser)
        errors.communalPurchaser = "Please select in a communal purchaser.";
    return errors;
};
...
Meteor.methods({
    ...
    settlementInsert : function(settlementAttributes){
        ...
        check(settlementAttributes, {
            ...
            purchaser: [Object],
            communalPurchaser: [Object],
            settlementCompleted: Boolean
        });

        var errors = validateSettlement(settlementAttributes);
        if(errors.dateOrdered || errors.dateSettlement || errors.category || errors.purchaser || errors.orderSummary || errors.orderPrice || errors.attachReceipt || errors.communalPurchaser)
            return Session.set('settlementSubmitErrors', errors);
